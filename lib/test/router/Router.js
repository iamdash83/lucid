...
module.exports = {
    ...
    test_correct_arguments: function(assert, done){
        var router = new Router();
        var wasCalled = false;
        ...
        router.add(/^\/foo\/([a-zA-Z]*)\/([0-9]*)\/$/, function(argFromCall, argFromRouter, anotherArgFromRouter){
            ...
            assert.pass("successfully matched against /foo/([a-zA-Z]*)/([0-9]*)/");
            done();
        });
    ...
    test_chained_routers: function(assert, done){
        ...
        router2.add(/^\/([a-zA-Z]*)\/$/, function(argFromCall, argFromRouter1, argFromRouter2){
            ...
            assert.pass("Successfully matched against /([a-zA-Z]*)/([a-zA-Z]*)/ with chained routers");
            done();
