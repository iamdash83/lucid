...
var AppContainer = module.exports = function(options){
    ...
    this._sandbox.on("stderr", function(err){
        ...
        //TODO: this sort of processing really should be done in the sandbox lib
        if(/\n$/.test(err)){
...
AppContainer.prototype.start = function(){
    //TODO: in sandbox, we should just have run() return a promise if possible
...
AppContainer.prototype.stop = function(){
    //TODO: shouldn't this return a promise as well?
    this._sandbox.kill();
}
...
AppContainer.prototype._getEventHandler = function(componentName){
    ...
    return function(){
        ...
        console.log(self._containerName+": got event from "+componentName+", need to deliver still!");
    }
}
