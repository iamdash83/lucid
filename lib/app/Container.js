...
var AppContainer = module.exports = function(options){
    ...
    options.sandboxDefaults = _.defaults(options.sandboxDefaults, sandboxDefaultsDefaults);
    ...
    this._packageName = options.appName;
...
AppContainer.prototype.start = function(){
    //TODO: in sandbox, we should just have run() return a promise
    
    var p = new Promise();

    var evt = this._sandbox.once("ready", function(){
    ...
    try {
        ...
        this._standbox.removeListener("ready", evt);
        p.errback(e);
    }
...
AppContainer.prototype._setupEvents = function(){
    ...
    for(var i in eventOptions){
        var h = this._componentManager.addMessageHandler(i, this._getEventHandler(i), packageName, eventOptions[i]);
