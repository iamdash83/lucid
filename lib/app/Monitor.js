...
//returns a list of modules that are in a given state `state`.
Monitor.prototype.getInState = function(state){
    var mods = [];
    for(var mod in this._statuses){
        if(this._statuses[mod] == state)
            mods.push(mod);
    }
