...
var AppManager = module.exports = function(options, componentManager, environment){
    ...
    this.getList(function(list){
        list.forEach(function(app){
            self.loadApp(app).then(function(containers){
                console.log("Started "+app+" successfully (containers: "+containers.join(", ")+")");
            }, function(err){
                throw err;
            });
        });
...
AppManager.prototype.loadApp = function(appName){
    if(!this._applications[appName]){
        this._applications[appName] = new Application({
            name: appName,
            path: path.join(this._options.path, appName),
