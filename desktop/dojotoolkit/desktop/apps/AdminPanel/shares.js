dojo.provide("desktop.apps.AdminPanel.shares");
...
dojo.extend(desktop.apps.AdminPanel, {
	shares: function(){
		...
		desktop.admin.shares.list(dojo.hitch(this, function(data){
			...
			dojo.connect(this._groupStore, "onDelete", this, function(a){
				desktop.admin.shares.remove(a.name[0]); //that feels really hackish
			})
	...
	shareMemberDialog: function(group){
		...
		var add = function(name) {
			desktop.dialog.input({title: this.win.title, message: sys.enterGroup, onComplete: dojo.hitch(this, function(name) {
				if(name == "") return;
				desktop.xhr({
